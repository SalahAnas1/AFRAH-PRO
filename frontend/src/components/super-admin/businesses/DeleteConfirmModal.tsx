...
export default function DeleteConfirmModal({ business, onConfirm, onCancel }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-white rounded-t-2xl sm:rounded-2xl p-6 w-full max-w-md shadow-2xl max-h-[92vh] overflow-y-auto">
        ...
        <button
          onClick={onCancel}
          className="absolute left-4 top-4 text-gray-text hover:text-dark transition-colors"
        >
          <X size={18} />
        </button>
        ...
        <h3 className="text-lg font-bold text-dark text-center mb-2">تأكيد حذف المحل</h3>

        <p className="text-gray-text text-sm text-center mb-1">هل أنت متأكد من حذف محل</p>
        <p className="text-dark font-bold text-center text-base mb-4">"{business.name}"؟</p>

        <div className="bg-red-50 rounded-xl px-4 py-2.5 mb-6">
          <p className="text-xs text-red-500 text-center">
            هذا الإجراء لا يمكن التراجع عنه. سيتم حذف جميع بيانات المحل نهائياً.
          </p>
        </div>
