...
import {
  Store, User, Bell, Shield, HardDrive, Settings2,
  Camera, Eye, EyeOff, Check, AlertCircle, Loader2, Globe,
} from "lucide-react";
...
function BusinessSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAlert({ type: "success", message: t("settings.business.saveSuccess") });
      setLogoFile(null); setCoverFile(null);
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? t("settings.business.saveError");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Field label={t("settings.business.coverImage")} hint={t("settings.business.coverHint")}>
        <div className="relative h-36 bg-gray-100 rounded-2xl overflow-hidden border-2 border-dashed border-gray-200 hover:border-rose-300 transition-colors cursor-pointer group"
          ...
          {coverPreview
            ...
            : <div className="flex flex-col items-center justify-center h-full gap-2 text-gray-400">
                ...
                <span className="text-sm">{t("settings.business.coverUpload")}</span>
              </div>
          }
          <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
            <span className="text-white text-sm font-medium flex items-center gap-2"><Camera size={16}/>{t("settings.business.coverChange")}</span>
          </div>
          <input ref={coverRef} type="file" accept="image/*" className="hidden" onChange={e => e.target.files?.[0] && handleFile(e.target.files[0], "cover")}/>
        </div>
      </Field>

      <Field label={t("settings.business.logo")} hint={t("settings.business.logoHint")}>
        <div className="flex items-center gap-4">
          ...
          <div className="text-sm text-gray-500">
            <p className="font-medium text-gray-700 mb-1">{t("settings.business.logo")}</p>
            <p className="text-xs text-gray-400">{t("settings.business.logoChange")}</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label={t("settings.business.businessName")}>
          <input className={inputCls} value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} required placeholder={t("settings.business.businessNamePlaceholder")}/>
        </Field>
        <Field label={t("settings.business.phone")}>
          <input className={inputCls} value={form.phone ?? ""} onChange={e => setForm(f => ({...f, phone: e.target.value}))} placeholder={t("settings.business.phonePlaceholder")} dir="ltr"/>
        </Field>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label={t("settings.business.email")}>
          <input className={inputCls} type="email" value={form.email ?? ""} onChange={e => setForm(f => ({...f, email: e.target.value}))} placeholder={t("settings.business.emailPlaceholder")} dir="ltr"/>
        </Field>
        <Field label={t("settings.business.city")}>
          <input className={inputCls} value={form.city ?? ""} onChange={e => setForm(f => ({...f, city: e.target.value}))} placeholder={t("settings.business.cityPlaceholder")}/>
        </Field>
      </div>

      <Field label={t("settings.business.address")}>
        <input className={inputCls} value={form.address ?? ""} onChange={e => setForm(f => ({...f, address: e.target.value}))} placeholder={t("settings.business.addressPlaceholder")}/>
      </Field>

      <Field label={t("settings.business.description")} hint={t("settings.business.descHint")}>
        <textarea className={`${inputCls} resize-none`} rows={3} value={form.description ?? ""} onChange={e => setForm(f => ({...f, description: e.target.value}))} placeholder={t("settings.business.descPlaceholder")}/>
      </Field>
      ...
      <div className="flex justify-end pt-2">
        <button type="submit" disabled={saving}
          ...
          {saving ? t("common.saving") : t("settings.business.saveBtn")}
        </button>
      </div>
    </form>
  );
...
function AccountSection() {
  ...
  const handleSaveAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAlert({ type: "success", message: t("settings.account.accountSuccess") });
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? t("common.errorOccurred");
      setAlert({ type: "error", message: msg });
    } finally { setSaving(false); }
  };
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    if (pwForm.password !== pwForm.password_confirmation) {
      setPwAlert({ type: "error", message: t("settings.account.passwordMismatch") }); return;
    }
    setSavingPw(true); setPwAlert(null);
    try {
      await settingsApi.changePassword(pwForm);
      setPwAlert({ type: "success", message: t("settings.account.passwordSuccess") });
      setPwForm({ current_password: "", password: "", password_confirmation: "" });
    } catch (err: unknown) {
      const errs = (err as { response?: { data?: { errors?: Record<string,string[]>; message?: string } } })?.response?.data;
      const msg  = errs?.errors?.current_password?.[0] ?? errs?.message ?? t("common.errorOccurred");
  ...
  return (
    <div className="space-y-8">
      <form onSubmit={handleSaveAccount} className="space-y-5">
        <h3 className="text-sm font-bold text-gray-700 pb-2 border-b border-gray-100">{t("settings.account.title")}</h3>
        {alert && <Alert type={alert.type} message={alert.message} onClose={() => setAlert(null)}/>}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label={t("settings.account.fullName")}>
            <input className={inputCls} value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} required placeholder={t("settings.account.namePlaceholder")}/>
          </Field>
          <Field label={t("settings.account.email")}>
            <input className={inputCls} type="email" value={form.email} onChange={e => setForm(f => ({...f, email: e.target.value}))} required placeholder={t("settings.account.emailPlaceholder")} dir="ltr"/>
          </Field>
        </div>
        <div className="flex justify-end">
          <button type="submit" disabled={saving}
            className="flex items-center gap-2 px-6 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-sm font-semibold transition-colors disabled:opacity-60">
            {saving ? <Loader2 size={15} className="animate-spin"/> : <Check size={15}/>}
            {saving ? t("common.saving") : t("settings.account.saveBtn")}
          </button>
        </div>
      </form>

      <form onSubmit={handleChangePassword} className="space-y-5">
        <h3 className="text-sm font-bold text-gray-700 pb-2 border-b border-gray-100">{t("settings.account.changePassword")}</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label={t("settings.account.newPassword")} hint={t("settings.account.passwordHint")}>
            <div className="relative">
        ...
        <div className="flex justify-end">
          <button type="submit" disabled={savingPw}
            ...
            {savingPw ? t("common.saving") : t("settings.account.changePwBtn")}
          </button>
        </div>
      </form>
    </div>
...
function LanguageSection() {
  ...
  const options: { value: Lang; label: string; desc: string; flag: string }[] = [
    { value: "ar", label: t("settings.language.arabic"), desc: t("settings.language.arabicDesc"), flag: "🇩🇿" },
    { value: "fr", label: t("settings.language.french"), desc: t("settings.language.frenchDesc"), flag: "🇫🇷" },
  ];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {options.map(opt => (
          <button
            ...
            className={`flex items-center gap-4 p-5 rounded-2xl border-2 text-right transition-all ${
              lang === opt.value
          ...
          >
            ...
            <div className="flex-1">
              <p className={`font-bold text-base ${lang === opt.value ? "text-rose-600" : "text-gray-800"}`}>
                {opt.label}
              </p>
...
// ─── Main Page ────────────────────────────────────────────────────────────────
type TabKey = "business" | "account" | "notifications" | "backup" | "system" | "language";

export default function SettingsPage() {
  const { t } = useLanguage();
  const [activeTab, setActiveTab] = useState<TabKey>("business");

  const tabs: { key: TabKey; labelKey: string; icon: React.ElementType; ready?: boolean }[] = [
    { key: "business",      labelKey: "settings.tabs.business",      icon: Store,    ready: true },
    { key: "account",       labelKey: "settings.tabs.account",       icon: User,     ready: true },
    { key: "language",      labelKey: "settings.tabs.language",      icon: Globe,    ready: true },
    { key: "notifications", labelKey: "settings.tabs.notifications", icon: Bell                  },
    { key: "backup",        labelKey: "settings.tabs.backup",        icon: HardDrive             },
    { key: "system",        labelKey: "settings.tabs.system",        icon: Settings2             },
  ];

  const sectionTitle: Record<TabKey, string> = {
    business:      t("settings.tabs.business"),
      ...
      <div className="p-3 sm:p-5">
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-5 items-start">
                ...
                return (
                  <li key={tab.key}>
                    <button onClick={() => setActiveTab(tab.key)} className={`w-full flex items-center gap-3 px-4 py-3 text-sm transition-colors ${
                      ...
                      <span className="flex-1 text-right">{t(tab.labelKey)}</span>
                      {!tab.ready && (
                        <span className="text-[9px] bg-gray-100 text-gray-400 px-1.5 py-0.5 rounded-full font-medium">{t("common.soon")}</span>
                      )}
                    </button>
                  </li>
                );
          ...
          <div className="flex-1 min-w-0 bg-white rounded-2xl border border-gray-200 p-4 sm:p-6">
            ...
            {activeTab !== "business" && activeTab !== "account" && activeTab !== "language" && <ComingSoon/>}
          </div>
        </div>
      </div>
    </div>
